...
/**
 * Class used to send fancy messages to the player; can be titles, hoverable
 * chat messages or action bar messages. All methods are static and this class
...
public final class FancyMessageSender {
    ...
    /**
     * Sends a clickable and hoverable message to the player. Only supported in
     * Minecraft 1.8+.
     *
     * @param player  Online player to send the message to.
     * @param message The text to display in the chat.
    ...
    /**
     * Sends an action bar chat message to the player. Only supported in Minecraft
     * 1.8+.
     *
     * @param player  Online player to send the message to.
     ...
     *                     http://minecraft.gamepedia.com/Commands#Raw_JSON_text for
     *                     more information.
     * @param subtitleJson The subtitle title JSON format message to send to the
     *                     player. See
     *                     http://minecraft.gamepedia.com/Commands#Raw_JSON_text for
     *                     more information.
            ...
            throws ReflectiveOperationException
    {
        ...
        if (PackageType.getServerVersion().startsWith("1_8_R1")) {
            ...
            parsedMainMessage = PackageType.MINECRAFT_SERVER.getClass(NESTED_CHAT_SERIALIZER)
                    .getMethod("a", String.class).invoke(null, UtilitiesOG.trueogExpand(titleJson, player));
            parsedSubMessage = PackageType.MINECRAFT_SERVER.getClass(NESTED_CHAT_SERIALIZER)
                    .getMethod("a", String.class).invoke(null, UtilitiesOG.trueogExpand(subtitleJson, player));
     ...
     *               http://minecraft.gamepedia.com/Commands#Raw_JSON_text for more
     *               information.
    ...
    private static void sendChatPacket(Player player, String json, byte type) throws ReflectiveOperationException {
        ...
        if (MINOR_VERSION_NUMBER < 12) {
            ...
            // public PacketPlayOutChat(IChatBaseComponent ichatbasecomponent,
            // ChatMessageType chatmessagetype, UUID
            // uuid)
    ...
    private static String constructHoverableMessageJson(String message, String hover, String color) {
        ...
        return "{\"text\":\"" + message.replace("\"", "\\\"") + "\","
                + "\"hoverEvent\":{\"action\":\"show_text\",\"value\":[{\"text\":\"" + hover.replace("\"", "\\\"")
                + "\",\"color\":\"" + color + "\"}]}}";

    }

    private static String constructHoverableCommandMessageJson(String message, String command, String hover,
            String color)
    {
